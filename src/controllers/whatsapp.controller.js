...
/**
 * PASO 4: Pregunta si quiere añadir un rango de precios.
 */
async function askForPriceConfirmation(userPhone, currentStateData) {
  conversationState.set(userPhone, { ...currentStateData, state: 'AWAITING_PRICE_CONFIRMATION' });
  const buttons = [
      { type: 'reply', reply: { id: `price_confirm:yes`, title: 'Sí, agregar precio' } },
      { type: 'reply', reply: { id: `price_confirm:no`, title: 'No, buscar así' } },
  ];
  await sendReplyButtonsMessage(userPhone, `¡Anotado! ¿Quieres filtrar por un rango de precios?`, buttons);
}

/**
 * PASO 3: Pregunta por la marca, usando una LISTA interactiva.
 */
async function askForBrand(userPhone, currentStateData) {
  ...
  const rows = suggestions.map(brand => ({ 
      id: `select_brand:${brand.id}`, 
      title: brand.title 
  })).slice(0, 10); // Max 10 filas
  
  conversationState.set(userPhone, { ...currentStateData, state: 'AWAITING_BRAND' });

  if (rows.length > 1) { // Si hay más que solo "Otras"
    await sendListMessage(userPhone, "Elige una Marca", `¡Perfecto! Buscaremos en *${category.toUpperCase()}*. ¿Tienes alguna marca en mente?`, "Marcas", [{ title: "Marcas Populares", rows }]);
  } else {
...
async function handleInteractiveReply(userPhone, message, currentStateData) {
  ...
  if (state === 'AWAITING_CATEGORY' && action === 'select_category') {
      ...
      if (category === 'otros') {
        ...
        // PASO 3: Pasa a preguntar la MARCA
        await askForBrand(userPhone, {
  ...
  if (state === 'AWAITING_BRAND' && action === 'select_brand') {
      if (payload === 'otras') {
          conversationState.set(userPhone, { state: 'AWAITING_CUSTOM_BRAND', data: searchContext });
          await sendTextMessage(userPhone, "Entendido, por favor escribe la marca que buscas:");
      } else {
          searchContext.brandPreference = payload;
          searchContext.query += ` ${payload}`;
          await askForPriceConfirmation(userPhone, { state: 'AWAITING_PRICE_CONFIRMATION', data: searchContext }); // PASO 4
      }
      return;
  }

  // PASO 4: Respuesta a la confirmación de precio
  if (state === 'AWAITING_PRICE_CONFIRMATION' && action === 'price_confirm') {
      if (payload === 'yes') {
          conversationState.set(userPhone, { ...currentStateData, state: 'AWAITING_PRICE_RANGE' });
          await sendTextMessage(userPhone, `Ok, ¿cuál es tu presupuesto? (ej: "hasta 150000", "entre 100k y 200k")`);
      } else {
  ...
  // PASO 6: Respuesta a la confirmación de análisis IA
      ...
      if (payload === 'yes') {
          ...
          await sendTextMessage(userPhone, "Entendido. ¡Aquí tienes los mejores 5 productos de mi análisis rápido!");
  ...
  if (message.type === 'text') {
    ...
    switch (currentStateData.state) {
        case 'AWAITING_CATEGORY':
             ...
             const queryBase = foundCategory ? foundCategory.query : userText;

             await askForBrand(userPhone, {
                state: 'AWAITING_BRAND', // PASO 3
                data: { ...currentSearchData, query: queryBase, category: categoryId }
            });
            break;
        ...
        case 'AWAITING_BRAND':
            ...
            if (userText.toLowerCase() !== 'ninguna') {
              currentSearchData.query = `${currentSearchData.query} ${userText}`;
            }
            await askForPriceConfirmation(userPhone, { state: 'AWAITING_PRICE_CONFIRMATION', data: currentSearchData }); // PASO 4
            break;

        // PASO 4: Usuario escribe el precio (después de decir "Sí")
